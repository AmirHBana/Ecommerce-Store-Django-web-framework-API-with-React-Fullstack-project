...
from store.serializers import ProductSerializer, CategorySerializer, CartSerializer, CartOrderSerializer, \
    CouponSerializer, Notification, NotificationSerializer, ReviewSerializer, WishListSerializer, WishList, \
    SummarySerializer, CartOrderItemSerializer, EarningSerializer, CouponSummarySerializer, \
    NotificationSummarySerializer, VendorSerializer, SpecificationSerializer, ColorSerializer, SizeSerializer, GallerySerializer
...
class EarningAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        one_month_ago = datetime.today() - timedelta(days=20)
...
class ShopUpdateView(generics.RetrieveUpdateAPIView):
    ...
    def get_serializer_context(self):
        context = super(ShopUpdateView, self).get_serializer_context()
        context['vendor_id'] = self.kwargs.get('pk')
        return context
